...
Stashing enables an actor to temporarily buffer all or some messages that can not or should not
...
Let's illustrate these two with an example. It's an actor that is used for like a single access point
...
When a new state is saved in the database it is also stashing incoming messages to make the
...
in the order they were added and all are processed. The actor is unresponsive to other new messages until
`unstashAll` is completed. That is another reason for keeping the number of stashed messages low. Actors
that hog the message processing thread for too long can result in starvation of other actors.
